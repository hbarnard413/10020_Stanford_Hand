/**
 * @file foe.xc
 * @brief  header for file file over EtherCAT access.
 */
...
unsigned foe_get_reply(uint16_t data[])
{
	...
	for (int i=0; (i<FOE_DATA_SIZE) && (i<(replySize-3)); i+=2) {
...
int foe_request(uint16_t data[])
{
	...
	for (int i=0; i<FOE_DATA_SIZE; i++, pos++) {
