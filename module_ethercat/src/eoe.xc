...
{
	for (int i=0; i<MAX_ETHERNET_FRAME_BYTES; i++) {
		ep.f.frameb[i]=0;
	}
...
void eoe_init(void)
{
	...
	for (int i=0; i<MAX_ETHERNET_BUFFER; i++) {
...
int eoe_rx_handler(chanend eoe, chanend sig, uint16_t msg[], unsigned size)
{
	...
	case EOE_FRAGMENT_REQ:
		...
		for (int i=0; i<packetSize && i<MAX_EOE_DATA; i++) {
		...
		if (inpacket.lastFragment == 1) {
			...
			//for (int i=0; i<(ethernet_packet_rx[0].size+3)/4; i++) {
			for (int i=0; i<(datalen+3)/4; i++) {
				eoe <: ethernet_packet_rx[0].f.framei[i];
			}

			/* Extended debug * /
			printstr("DEBUG Packet received: ");
			for (int i=0; i<ethernet_packet_rx[0].size; i++) {
				printhex(ethernet_packet_rx[0].f.frameb[i]);
				printstr(" ");
			}
			printstr("\n");
